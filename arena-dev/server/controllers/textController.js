...
const startJob = async (prompt, modelString) => {
  const res = await fetch("https://api.edenai.run/v3/universal-ai/async", {
    ...
    body: JSON.stringify({ model: modelString, input: prompt }),
  });
...
export const generate = async (req, res) => {
  try {
    ...
    const [modelAId, modelBId] = getLeastPlayedPair("text");
    ...
    const textA = outputA.output?.[0]?.content?.[0]?.text
    const textB = outputB.output?.[0]?.content?.[0]?.text
    
    const battleResult = db.prepare(`
      INSERT INTO battles (prompt_id, model_a_id, model_b_id, output_a, output_b)
      VALUES (?, ?, ?, ?, ?)
    `).run(promptId, modelA.id, modelB.id, textA, textB);
